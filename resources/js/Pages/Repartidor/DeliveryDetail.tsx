...
export default function DeliveryDetail({ auth, entrega, errors: serverErrors, flash }: any) {
    ...
    const [intentosRestantes, setIntentosRestantes] = useState(3 - (entrega.intentos_fallidos || 0));
    const [localError, setLocalError] = useState('');
    const [showBlockOptions, setShowBlockOptions] = useState(false);
    const [isBlocking, setIsBlocking] = useState(false);

    const handleBlock = (motivo: string) => {
        setIsBlocking(true);
        router.post(route('repartidor.delivery.block', entrega.id), { motivo }, {
    ...
    return (
        <AppLayout>
            ...
            <div className="max-w-md mx-auto pt-6 pb-12 px-5 font-['Montserrat',sans-serif]">
                ...
                {/* Botón Volver */}
                <button onClick={handleBack} className="flex items-center gap-2 text-slate-400 hover:text-[#c2185b] font-semibold text-xs mb-6 transition-colors">
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 mb-4">
                    <h3 className="text-[#e91e63] font-semibold text-sm mb-1">Información del cliente:</h3>
                    <p className="font-bold text-slate-800 text-sm">{entrega.cliente || 'Sin nombre registrado'}</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative">
                    {intentosRestantes <= 0 && (
                        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm rounded-3xl flex items-center justify-center z-10 p-6 text-center">
                            <div>
                                ...
                                <h3 className="font-semibold text-slate-800 text-lg">Bloqueado</h3>
                                <p className="text-sm text-slate-500 mt-1">Has superado los 3 intentos. Por favor contacta a soporte.</p>
                            </div>
                        </div>
                    )}

                    <h3 className="text-[#e91e63] font-semibold text-base mb-4">Entregar:</h3>
                    ...
                    <form onSubmit={submit}>
                        ...
                        <button 
                            type="submit"
                            disabled={processing || !data.palabra_clave || intentosRestantes <= 0 || isBlocking}
                            className="w-full py-3.5 bg-[#e91e63] text-white rounded-full font-semibold text-base hover:bg-opacity-90 transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                    ...
                    {/* Botón No pude entregar */}
                    <div className="mt-4">
                        {!showBlockOptions ? (
                            <button 
                                type="button"
                                onClick={() => setShowBlockOptions(true)}
                                disabled={processing || isBlocking || intentosRestantes <= 0}
                                className="w-full py-3 bg-white text-slate-500 border border-slate-200 rounded-full font-semibold text-sm hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                No pude entregar
                            </button>
                        ) : (
                            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 mt-2 animate-fade-in-up">
                                <div className="flex justify-between items-center mb-3">
                                    <h4 className="text-sm font-bold text-slate-700">Selecciona el motivo:</h4>
                                    <button onClick={() => setShowBlockOptions(false)} className="text-slate-400 hover:text-slate-600">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                </div>
                                <div className="space-y-2">
                                    {['Cliente no contesta', 'Cliente no se encuentra', 'Dirección errada'].map(motivo => (
                                        <button 
                                            key={motivo}
                                            type="button"
                                            onClick={() => handleBlock(motivo)}
                                            disabled={isBlocking}
                                            className="w-full py-2.5 px-4 bg-white text-left text-sm font-semibold text-slate-600 border border-slate-200 rounded-xl hover:bg-pink-50 hover:text-[#e91e63] hover:border-pink-200 transition-colors disabled:opacity-50 flex justify-between items-center group"
                                        >
                                            {motivo}
                                            {isBlocking ? (
                                                <svg className="animate-spin w-4 h-4 text-[#e91e63] opacity-0 group-hover:opacity-100" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                            ) : (
                                                <svg className="w-4 h-4 text-slate-300 group-hover:text-[#e91e63] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                                            )}
                                        </button>
                                    ))}
                                </div>
