...
export default function Assign({ auth, flash, entregas = [] }: any) {
    ...
    const [activeTab, setActiveTab] = useState<'por_entregar' | 'entregados' | 'bloqueados'>('por_entregar');
    ...
    const entregasFiltradas = entregas.filter((entrega: any) => {
        if (activeTab === 'por_entregar') {
            return entrega.estado !== 'entregado' && entrega.estado !== 'bloqueado';
        } else if (activeTab === 'entregados') {
            return entrega.estado === 'entregado';
        } else {
            return entrega.estado === 'bloqueado';
        }
    });
        ...
        <AppLayout>
            ...
            <div className="max-w-md mx-auto pt-6 pb-12 px-5">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-[#c2185b] text-2xl font-bold tracking-tight leading-none">Panel de Entregas</h1>
                ...
                {/* Subheader */}
                <div className="mb-6">
                    <h2 className="text-[#c2185b] text-lg font-bold tracking-tight">Resumen del Repartidor</h2>
                    <p className="text-slate-500 text-xs font-medium mt-0.5">Seguimiento de entregas asignadas</p>
                </div>

                {/* Mensajes Flash */}
                {flash?.status && (
                    <div className="bg-[#f0fdf4] text-[#166534] p-4 rounded-2xl mb-6 border border-green-200 font-semibold text-sm text-center shadow-sm flex items-center justify-center gap-2">
                ...
                {flash?.error && (
                    <div className="bg-red-50 text-red-700 p-4 rounded-2xl mb-6 border border-red-200 font-semibold text-sm text-center shadow-sm">
                        {flash.error}
                    </div>
                )}
                ...
                <div className="flex gap-6 border-b border-slate-200 mb-6 px-1">
                    <button 
                        ...
                        className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'por_entregar' ? 'text-[#c2185b]' : 'text-slate-400 hover:text-[#c2185b]'}`}
                    >
                        Por entregar
                    ...
                    <button 
                        ...
                        className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'entregados' ? 'text-[#c2185b]' : 'text-slate-400 hover:text-[#c2185b]'}`}
                    >
                        Entregados
                    ...
                    <button 
                        ...
                        className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'bloqueados' ? 'text-[#c2185b]' : 'text-slate-400 hover:text-[#c2185b]'}`}
                    >
                        Bloqueados
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {activeTab === 'por_entregar' && (
                        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-2">
                            <form onSubmit={submit}>
                                ...
                                {errors.tracking_id && <div className="text-red-500 text-xs font-semibold mt-2 ml-4">{errors.tracking_id}</div>}
                            </form>
                        </div>
                    )}
                    ...
                    {entregasFiltradas.length > 0 ? (
                        entregasFiltradas.map((entrega: any) => (
                                <div 
                                    ...
                                    onClick={() => {
                                        ...
                                        } else if (entrega.estado !== 'bloqueado') {
                                            router.get(route('repartidor.delivery.detail', entrega.id));
                                        }
                                    }}
                                    className={`bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between transition-all ${entrega.estado !== 'bloqueado' ? 'cursor-pointer hover:shadow-md active:scale-[0.98]' : 'opacity-80'}`}
                                >
                                    <div className="flex items-center gap-4">
                                        {/* Icono de estado */}
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 shrink-0 
                                            ${entrega.estado === 'entregado' ? 'border-green-100 bg-green-50 text-green-500' : 
                                              entrega.estado === 'bloqueado' ? 'border-red-100 bg-red-50 text-red-500' : 
                                        ...
                                        {/* Info de la entrega */}
                                        <div>
                                            <div className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full inline-block mb-1 
                                                ${entrega.estado === 'entregado' ? 'bg-green-100 text-green-600' : 
                                                  entrega.estado === 'bloqueado' ? 'bg-red-100 text-red-600' : 
                                                  'bg-orange-100 text-orange-600'}`}>
                                                {entrega.estado === 'entregado' ? 'Entregado' : 
                                                 entrega.estado === 'bloqueado' ? 'Bloqueado' : 
                                                 'Por Entregar'}
                                            </div>
                                            <h3 className="text-sm font-bold text-slate-800 m-0">Guía # {entrega.tracking_id}</h3>
                                            <p className="text-xs text-slate-500 font-medium my-0.5">{entrega.cliente || 'Sin nombre de cliente'}</p>
                                            
                                            {entrega.estado === 'bloqueado' && (
                                                <p className="text-[10px] text-red-500 font-bold m-0 mt-1 uppercase">Motivo: {entrega.motivo_bloqueo}</p>
                                            )}
                                            {entrega.estado !== 'entregado' && entrega.estado !== 'bloqueado' && (
                        ...
                        <div className="text-center py-10 px-6 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-100 mt-2">
                            ...
                            <p className="text-slate-400 font-semibold text-sm">No tienes entregas en esta lista.</p>
                        </div>
                    )}
                </div>
            </div>
