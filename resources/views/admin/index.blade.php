                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem;">
                    ...
                    <div style="background: #f1f5f9; padding: 1rem; border-radius: 0.75rem; display: flex; gap: 2rem;">
                        <div style="text-align: center;">
                            ...
                            <p style="margin: 0; font-size: 1.5rem; font-weight: 800; color: #2563eb;">{{ $entregas->total() }}</p>
                        </div>
                        <div style="width: 1px; background: #cbd5e1;"></div>
                        <div style="text-align: center;">
                            <p style="margin: 0; font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase;">Repartidores</p>
                            <p style="margin: 0; font-size: 1.5rem; font-weight: 800; color: #1e293b;">{{ $repartidores->count() }}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.1s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                                        ...
                                        <div>
                                            <p style="margin: 0; font-weight: 700; color: #1e293b; font-size: 0.95rem;">{{ $entrega->user->name }}</p>
                                ...
                                <td style="padding: 1.25rem 1.5rem;">
                                    @if($entrega->url_evidencia)
                                        <div style="position: relative; width: 60px; height: 60px; border-radius: 0.5rem; overflow: hidden; border: 2px solid #fff; box-shadow: 0 0 0 1px #e2e8f0; cursor: pointer;" onclick="window.open('{{ $entrega->url_evidencia }}', '_blank')">
                                            <img src="{{ $entrega->url_evidencia }}" style="width: 100%; height: 100%; object-fit: cover;">
                                        </div>
                                    @else
                                ...
                                <td style="padding: 1.25rem 1.5rem;">
                                    <p style="margin: 0; font-size: 0.9rem; font-weight: 700; color: #1e293b;">{{ $entrega->created_at->format('d/m/Y') }}</p>
                                    <p style="margin: 0; font-size: 0.75rem; color: #64748b;">{{ $entrega->created_at->format('H:i:s') }}</p>
                                </td>
                                <td style="padding: 1.25rem 1.5rem;">
                                    <div style="display: flex; gap: 0.5rem;">
                                        <a href="{{ $entrega->url_evidencia }}" target="_blank" style="background: #f1f5f9; color: #1e293b; padding: 0.4rem 0.8rem; border-radius: 0.4rem; text-decoration: none; font-size: 0.75rem; font-weight: 700; border: 1px solid #cbd5e1;">VER</a>
                                        <a href="{{ $entrega->url_evidencia }}" download style="background: #2563eb; color: white; padding: 0.4rem 0.8rem; border-radius: 0.4rem; text-decoration: none; font-size: 0.75rem; font-weight: 700;">BAJAR</a>
                                    </div>
                                </td>
                            </tr>
                        @empty
